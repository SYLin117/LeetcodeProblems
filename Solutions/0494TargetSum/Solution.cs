namespace Solutions._0494TargetSum;
...
public class Solution
{
    public int FindTargetSumWays(int[] nums, int target)
    {
        ...
        // we divide nums into two groups left and right, and assume left is the bigger one
        // left - right = target
        ...
        // dp[i][j]：遍历到数组第i个数时， left为j时的能装满背包的方法总数(dp[i][j] == j的方法總數)
        int[,] dp = new int[nums.Length, left + 1];

        // 初始化最上ROW（dp[0][j])，当nums[0] == j时（注意nums[0]和j都一定是大于等于零的，因此不需要判断等于-j时的情况），有唯一一种取法可取到j，dp[0][j]此时等于1
        // 其他情况dp[0][j] = 0
        // java整数数组默认初始值为0
        ...
        // 初始化最左COL（dp[i][0]), 背包大小為0, left為0的方法數
